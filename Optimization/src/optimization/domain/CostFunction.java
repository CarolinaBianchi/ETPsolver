...
public interface CostFunction {
     ...
     * int penalty = 0; int distance = Math.abs(indexes[0] - indexes[1]); if
     * (distance > 5) { return 0; } for (Exam e :
     * timeslots[indexes[0]].getExams()) { penalty +=
     * timeslots[indexes[1]].conflictWeight(e.getConflictingExams2()) *
     * COST[distance]; } return penalty; }
     */
    /**
     ...
     * @param indexes
     *
     * static void reorder(int[] indexes) { int tmp; if (indexes[0] >
     * indexes[1]) { tmp = indexes[0]; indexes[0] = indexes[1]; indexes[1] =
     * tmp; }
     *
     * }
     */
